...
import "./libraries/SafeMathSwapFee.sol";
...
interface ICafeSwapV2Token is IERC20 {
...
contract SwapFeeReward is Ownable {
    ...
    ICafeSwapV2Token public CafeToken;
    ...
    {
        require(tokenA != tokenB, "CafeSwapFactory: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "CafeSwapFactory: ZERO_ADDRESS");
    }
        ...
        returns (uint256 swapFee)
    {
        swapFee = uint256(1000).sub(
            ICafeSwapV2Pair(pairFor(tokenA, tokenB)).swapFee()
        );
    }
    ...
    function withdraw(
        ...
        if (balance > 0) {
            CafeToken.mint(msg.sender, balance);
    ...
    function getQuantity(
        ...
        if (outputToken == anchorToken) {
            ...
            ICafeSwapV2Factory(factory).getPair(outputToken, anchorToken) !=
            address(0) &&
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (
                    ICafeSwapV2Factory(factory).getPair(
                        outputToken,
                        intermediate
                    ) !=
                    address(0) &&
