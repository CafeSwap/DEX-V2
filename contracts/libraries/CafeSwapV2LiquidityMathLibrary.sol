...
import "./SafeMath.sol";
import "./CafeSwapV2Library.sol";

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library CafeSwapV2V2LiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = CafeSwapV2Library.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ...
        require(
            ...
            "CafeSwapV2ArbitrageLibrary: ZERO_PAIR_RESERVES"
        );
        ...
        if (aToB) {
            uint256 amountOut = CafeSwapV2Library.getAmountOut(
                amountIn,
                reserveA,
                reserveB,
                ICafeSwapV2Pair(
                    CafeSwapV2Library.pairFor(factory, tokenA, tokenB)
                ).swapFee()
            );
            reserveA += amountIn;
            reserveB -= amountOut;
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = CafeSwapV2Library.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ...
        bool feeOn = ICafeSwapV2Factory(factory).feeTo() != address(0);
