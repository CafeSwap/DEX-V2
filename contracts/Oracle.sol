...
import "./libraries/CafeSwapV2OracleLibrary.sol";
...
contract Oracle {
    ...
    function update(address tokenA, address tokenB) external {
        require(
            ...
            "CafeOracle: Price can update only price updater address"
        );
        address pair = pairFor(tokenA, tokenB);

        Observation storage observation = pairObservations[pair];
        uint256 timeElapsed = block.timestamp - observation.timestamp;
        require(timeElapsed >= CYCLE, "CafeOracle: PERIOD_NOT_ELAPSED");
        (
            uint256 price0Cumulative,
            uint256 price1Cumulative,

        ) = CafeSwapV2OracleLibrary.currentCumulativePrices(pair);
